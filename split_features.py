...
def split_features(filename: str, sample_name:str, rt_window: float, num_splits: int):
    features = pd.read_csv(filename).fillna(0).sort_values('retention_time')
    ...
    for i, split in enumerate(splits):
        pd.concat(split).to_csv(filename.replace(ext, '_{}{}'.format(i, ext)),
                                index=False)
    output_filenames = filename.replace(ext, '_{}{}'.format(i, ext))
